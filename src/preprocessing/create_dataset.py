...
class MOSI:
    def __init__(self, sdk_dir=None, dataset_dir=None, mosi_data_path=None, mosi_label_path=None):
        if sdk_dir is None:
        ...
        try:
            ...
            with open(mosi_data_path, 'rb') as f:
                d = pickle.load(f)
            
            # read csv file for label and text
            df = pd.read_csv(mosi_label_path)
            text = df['text']
            vid = df['video_id']
            cid = df['clip_id']
