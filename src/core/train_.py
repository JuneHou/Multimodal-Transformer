...
def evaluate_irg(args, device, data_loader, model, mode=None):
    ...
    if 'ihm' in args.task:
        ...
        eval_vals['f1'] = eval_val
        if mode=='val' and args.finish_train==False:
            check_point(eval_vals, model, eval_probs, args, "f1")
...
def update_kl_weights(args,epoch,smooth_factor,datasets):
    ...
    if f'new_weights/{args.modeltype}' not in args.file_path:
        output_dir = os.path.join(args.file_path, f'new_weights/{args.modeltype}/')
