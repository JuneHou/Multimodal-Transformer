...
def trainer_irg(model,args,accelerator,train_dataloader,dev_dataloader,test_data_loader,device,optimizer,pretrain_epoch=None,writer=None,scheduler=None):
    ...
    for epoch in tqdm(range(args.num_train_epochs)):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            ids, ts_input_sequences, ts_mask_sequences, ts_tt, reg_ts, input_ids_sequences, attn_mask_sequences, text_emb, note_time, note_time_mask, cxr_feats, cxr_time, cxr_time_mask, ecg_feats, ecg_time, ecg_time_mask, label, cxr_missing, text_missing, ecg_missing = batch

            if  args.modeltype == "TS_Text":
                loss, probs=model(x_ts=ts_input_sequences, \
                ...
                loss, probs=model(x_ts=ts_input_sequences, \
                        ...
                        ecg_time=ecg_time, \
                        ecg_time_mask=ecg_time_mask,labels=label,reg_ts=reg_ts,\
                        cxr_missing=cxr_missing, ecg_missing=ecg_missing)
            elif args.modeltype == 'TS_ECG_Text':
                loss, probs=model(x_ts=ts_input_sequences, \
                        x_ts_mask=ts_mask_sequences,\
                        ts_tt_list=ts_tt,\
                        input_ids_sequences=input_ids_sequences,\
                ...
                loss, probs=model(input_ids_sequences=input_ids_sequences,\
                ...
                loss, probs=model(cxr_feats=cxr_feats,\
                        cxr_time=cxr_time, \
...
def evaluate_irg(args, device, data_loader, model, mode=None):
    ...
    for idx, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if  args.modeltype == "TS_Text":
                proj, probs=model(x_ts=ts_input_sequences, \
                        x_ts_mask=ts_mask_sequences,\
                        ts_tt_list=ts_tt,\
                        input_ids_sequences=input_ids_sequences,\
                        attn_mask_sequences=attn_mask_sequences, text_emb=text_emb, note_time_list=note_time,\
                ...
                #probs=model(input_ids_sequences=input_ids_sequences,\
                #        attn_mask_sequences=attn_mask_sequences)
                proj, probs=model(input_ids_sequences=input_ids_sequences,\
                        attn_mask_sequences=attn_mask_sequences, text_emb=text_emb,\
                        note_time_list=note_time, note_time_mask_list=note_time_mask,\
                        text_missing=text_missing)
            elif args.modeltype == "CXR":
                proj, probs=model(cxr_feats=cxr_feats,\
                        cxr_time=cxr_time, \
                        cxr_time_mask=cxr_time_mask, cxr_missing=cxr_missing)
            elif args.modeltype == "ECG":
                proj, probs=model(ecg_feats=ecg_feats,\
                        ecg_time=ecg_time, \
